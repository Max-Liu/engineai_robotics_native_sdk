#include "rl_bilidance_tracking/rl_bilidance_tracking_runner.h"
...
struct RlBilidanceTrackingRunner::ReferenceState {
...
class RlBilidanceTrackingRunner::TrackingPolicyModel {
public:
  TrackingPolicyModel(const std::string &model_path, int obs_dim,
                      int num_actions) {
    if (!std::filesystem::exists(model_path)) {
      throw std::runtime_error("Bilidance policy file does not exist: " +
                               model_path);
    }
...
RlBilidanceTrackingRunner::~RlBilidanceTrackingRunner() = default;

void RlBilidanceTrackingRunner::SetupContext() {
  data_store_->parallel_by_classic_parser.store(false);
}

void RlBilidanceTrackingRunner::TeardownContext() {}

bool RlBilidanceTrackingRunner::Enter() {
  if (!param_tag_.empty()) {
    param_ =
        data::ParamManager::create<data::RlBilidanceTrackingParam>(param_tag_);
  }
  if (!param_) {
    LOG(ERROR) << "[RlBilidanceTrackingRunner::Enter] Failed to load parameters";
    return false;
  }
  ...
  try {
    ...
    LOG(ERROR) << "[RlBilidanceTrackingRunner::Enter] " << error.what();
    return false;
  }

  LOG(INFO) << "[RlBilidanceTrackingRunner::Enter] Loaded Bilidance policy, "
...
void RlBilidanceTrackingRunner::Run() {
  if (!policy_) {
    LOG(ERROR) << "[RlBilidanceTrackingRunner::Run] Policy is not loaded";
    return;
  }
  ...
  try {
    ...
    if (iter_ < 5 || iter_ % 50 == 0) {
      LOG(INFO) << "[DEBUG-BILIDANCE] iter=" << iter_
    ...
    LOG(ERROR) << "[RlBilidanceTrackingRunner::Run] " << error.what();
    SetRunnerState(RunnerState::kTryExit);
  }
}

TransitionState RlBilidanceTrackingRunner::TryExit() {
  return TransitionState::kCompleted;
}

bool RlBilidanceTrackingRunner::Exit() { return true; }

void RlBilidanceTrackingRunner::End() {}

void RlBilidanceTrackingRunner::InitializeJointMapping() {
  ...
  if (param_->joint_stiffness.size() != num_actions ||
    ...
    throw std::runtime_error(
        "Bilidance joint parameter vector sizes must match joint_names");
  }
  ...
  for (int i = 0; i < num_actions; ++i) {
    ...
    if (!model_param_->joint_id_in_total_limb.contains(joint_name)) {
      throw std::runtime_error("Unknown SDK joint name in Bilidance config: " +
                               joint_name);
    }
...
void RlBilidanceTrackingRunner::InitializeObservationBuffers() {
  ...
  if (single_frame_dim != 161) {
    throw std::runtime_error(
        "Unexpected Bilidance single-frame observation dim: " +
        std::to_string(single_frame_dim));
  }
}

void RlBilidanceTrackingRunner::InitializeReferenceAlignment() {
  ...
  LOG(INFO) << "[RlBilidanceTrackingRunner::InitializeReferenceAlignment] "
            << "reference anchor aligned to robot anchor, translation=("
            << reference_alignment_pos_.transpose() << ")";
}

void RlBilidanceTrackingRunner::ResetObservationBuffers() {
...
void RlBilidanceTrackingRunner::ReadCurrentState() {
...
RlBilidanceTrackingRunner::ReferenceState
RlBilidanceTrackingRunner::SampleReference(int time_step) {
  ReferenceState reference = policy_->RunReference(zero_observation_, time_step);
  ApplyReferenceAlignment(reference);
  return reference;
}

void RlBilidanceTrackingRunner::ApplyReferenceAlignment(
...
Eigen::VectorXf
RlBilidanceTrackingRunner::BuildObservation(const ReferenceState &reference) {
...
Eigen::VectorXf
RlBilidanceTrackingRunner::ProcessObservationTerm(size_t term_index,
...
int RlBilidanceTrackingRunner::GetObservationDim(const std::string &name) const {
  if (name == "command")
...
int RlBilidanceTrackingRunner::ComputeObservationDim() const {
  int dim = 0;
  for (size_t i = 0; i < param_->observation_names.size(); ++i) {
...
void RlBilidanceTrackingRunner::CalculateWarmupMotorCommand(
  ...
  if (iter_ < 5 || iter_ % 50 == 0) {
    LOG(INFO) << "[DEBUG-BILIDANCE] warmup iter=" << iter_
              << " alpha=" << alpha
...
void RlBilidanceTrackingRunner::CalculateMotorCommand(
    const Eigen::VectorXf &action) {
  if (action.size() != param_->num_actions) {
    throw std::runtime_error("Bilidance policy action size mismatch");
  }
...
void RlBilidanceTrackingRunner::SendMotorCommand() {
  qd_des_ = Eigen::VectorXd::Zero(model_param_->num_total_joints);
  tau_ff_des_ = Eigen::VectorXd::Zero(model_param_->num_total_joints);
  GetMutableOutput().SetCommand(q_des_, qd_des_, joint_kp_, joint_kd_,
                                tau_ff_des_);
...
void RlBilidanceTrackingRunner::AdvanceTimeStep() {
  ++iter_;
...
data::LinkInfo RlBilidanceTrackingRunner::GetRobotAnchorState() const {
