...
#include "rl_bilidance_tracking_param/rl_bilidance_tracking_param.h"

namespace runner {

class RlBilidanceTrackingRunner : public MotionRunner {
public:
  RlBilidanceTrackingRunner(
      std::string_view name,
      const std::shared_ptr<data::DataStore> &data_store)
      : MotionRunner(name, data_store) {
    param_ = data::ParamManager::create<data::RlBilidanceTrackingParam>();
  }
  ~RlBilidanceTrackingRunner() override;
...
private:
  ...
  std::shared_ptr<data::RlBilidanceTrackingParam> param_;
...
REGISTER_RUNNER(RlBilidanceTrackingRunner, "rl_bilidance_tracking_runner",
                kMotion)
