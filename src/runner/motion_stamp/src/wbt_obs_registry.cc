...
// If a new data source is needed, add it to ObsContext in motion_stamp_obs_registry.h
// and populate it in MotionStampRunner::fillObsContextConstantPart.
...
static void ComputeImuInBody(const ObsContext& ctx, Eigen::Matrix3d* R_real, Eigen::Vector3d* w_real,
  ...
  if (R_real) R_real->setIdentity();
  if (w_real) w_real->setZero();
  if (projected_gravity) *projected_gravity = -Eigen::Vector3d::UnitZ();
...
static data::SE3Frame GetBaseFrame(const ObsContext& ctx) {
  if (!ctx.data_store) {
    return data::SE3Frame();
  }
  return ctx.data_store->base_state_in_world.Get()->frame;
}
  ...
  if (registry.empty()) {
    ...
    registry["motion_anchor_pos_b"] = {
        ...
        [](const ObsContext& ctx) -> Eigen::VectorXd {
          if (!ctx.ref_body_pos_w_all || ctx.ref_body_pos_w_all->rows() == 0) {
            LOG(ERROR) << "[obs:motion_anchor_pos_b] ref_body_pos_w_all is null or empty";
            return Eigen::VectorXd::Zero(3);
          }
          int r = ctx.ref_body_pos_w_all->rows();
          int step = std::min(ctx.policy_step, r - 1);
          Eigen::Vector3d ref_pos_w = ctx.ref_body_pos_w_all->row(step).transpose();
          Eigen::Vector3d body_pos_w = GetBaseFrame(ctx).pose.position;

          Eigen::Matrix3d R_real;
          Eigen::Vector3d w_unused, proj_g_unused;
          ComputeImuInBody(ctx, &R_real, &w_unused, &proj_g_unused);
          Eigen::Matrix3d body_aligned = ctx.body_init_yaw_rot.transpose() * R_real;

          Eigen::Vector3d ref_aligned_pos = ctx.ref_init_yaw_rot.transpose() * (ref_pos_w - ctx.ref_init_pos_w);
          Eigen::Vector3d body_aligned_pos = ctx.body_init_yaw_rot.transpose() * (body_pos_w - ctx.body_init_pos_w);
          Eigen::Vector3d anchor_pos_b = body_aligned.transpose() * (ref_aligned_pos - body_aligned_pos);
          return anchor_pos_b;
        },
    };
    ...
    registry["base_lin_vel"] = {
        ...
        [](const ObsContext& ctx) {
          ...
          Eigen::Vector3d w_unused, proj_g_unused;
          ComputeImuInBody(ctx, &R_real, &w_unused, &proj_g_unused);
          return R_real.transpose() * GetBaseFrame(ctx).twist.linear;
        },
    };
